...
import sql_stock_queries_nike # needed so i can access the functions from the file
...
for item in output["data"]["products"]["products"]:
    for item in output["data"]["products"]["products"]:
        ...
        if item["productType"] == "FOOTWEAR" and ("jordan" in name_of_shoe.lower()) == False:
            item_url= item["url"]
            item_url = str(item_url).replace("{countryLang}", "nike.com")
            
            sql_stock_queries_nike.search_for_dupes(item["id"],item["inStock"])
            
            sql_stock_queries_nike.insert_into_shoe_table(item,item_url)
